...
// Hook para detectar si hay un modal/Sheet abierto en móvil
const useModalOpen = () => {
  ...
  useEffect(() => {
    ...
    const checkModals = () => {
      // Detecta Sheet, Dialog, Modal abiertos
      const hasSheet = document.querySelector('[role="dialog"]');
      const hasBackdrop = document.querySelector('[data-state="open"]');
      setIsModalOpen(!!(hasSheet || hasBackdrop));
    };
...
const Sidebar = ({ onLogout }) => {
  ...
  const isModalOpen = useModalOpen(); // Detectar modales en móvil
  ...
  const allNavItems = [
    {
      id: 'dashboard', label: 'Dashboard', roles: ['SUPER_ADMIN', 'ADMIN', 'EMPLEADO'],
    ...
    {
      id: 'productos', label: 'Productos', roles: ['SUPER_ADMIN', 'ADMIN', 'EMPLEADO'],
    ...
    {
      id: 'stock', label: 'Stock', roles: ['SUPER_ADMIN', 'ADMIN', 'EMPLEADO'],
      iconClass: 'nav-icon-stock',
      icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" /><polyline points="3.27 6.96 12 12.01 20.73 6.96" /><line x1="12" y1="22.08" x2="12" y2="12" /></svg>
    },
    {
      id: 'proveedores', label: 'Proveedores', roles: ['SUPER_ADMIN', 'ADMIN'],
    ...
    {
      id: 'reportes', label: 'Reportes', roles: ['SUPER_ADMIN', 'ADMIN'],
    ...
    {
      id: 'configuracion', label: 'Configuración', roles: ['SUPER_ADMIN', 'ADMIN'],
  ...
  return (
    <>
      {/* ── Botones arriba solo en MÓVIL ── */}
      <div id="mobile-top-buttons" className={cn(
        "md:hidden fixed top-4 left-4 right-4 z-[1000000] flex justify-between pointer-events-none transition-opacity duration-300",
        isModalOpen && "opacity-0 pointer-events-none"
      )}>
        <div
          className={cn(
            "pointer-events-auto flex h-[48px] w-[48px] shrink-0 cursor-pointer items-center justify-center rounded-full border backdrop-blur-[16px] shadow-[0_4px_20px_rgba(0,0,0,0.3)] transition-all duration-[400ms] hover:scale-105",
            isDarkMode
        ...
        >
          {isDarkMode ? (
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]">
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-slate-700 drop-shadow-[0_2px_4px_rgba(15,23,42,0.3)]">
        ...
        <div
          ...
          title="Cerrar sesión"
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={cn(
      ...
      <aside
        ...
        className={cn(
          ...
          // Mobile: solo la burbuja del menú centrada abajo
          'bottom-4 left-1/2 -translate-x-1/2 flex-row items-end gap-4 md:gap-0',
          // Ocultar en móvil cuando hay modal abierto
          isModalOpen && 'max-md:opacity-0 max-md:pointer-events-none'
        )}
      >
      <div
        className={cn(
          "sidebar-bubble relative hidden md:flex h-[55px] w-[55px] md:h-[65px] md:w-[65px] shrink-0 cursor-pointer items-center justify-center rounded-full border backdrop-blur-[16px] shadow-[0_4px_20px_rgba(0,0,0,0.3)] transition-all duration-[400ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:scale-105",
          isDarkMode
            ? "border-white/10 bg-white/5 hover:border-white/20 hover:bg-white/10"
            : "border-slate-200/50 bg-white/80 hover:border-slate-300 hover:bg-white"
        )}
