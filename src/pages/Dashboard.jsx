...
import {
  ...
  Box, CheckCircle2, ArrowUpRight, Truck,
...
// Componentes de dashboard extraídos
import { LiveClock }    from '../components/dashboard/LiveClock'
import { StockBar }     from '../components/dashboard/StockBar'
import { LowStockItem } from '../components/dashboard/LowStockItem'
import { EstadoPill }   from '../components/dashboard/EstadoPill'
import { MovRow }       from '../components/dashboard/MovRow'
...
const Dashboard = () => {
  ...
  const lowItems        = useMemo(() => [...productos].filter(p => p.stock <= (p.stockMinimo || 5)).sort((a, b) => a.stock - b.stock), [productos])
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray('.gsap-stock-bar-fill').forEach(bar => {
        gsap.fromTo(bar, { width: '0%' }, {
          width: `${bar.dataset.pct}%`, duration: 1.4, ease: 'power3.out',
    ...
    <div ref={dashRef} className="dashboard-aurora-bg relative min-h-screen overflow-hidden">
      ...
      >
            ...
            <div className="relative px-8 pb-8 pt-8 lg:px-10 lg:pt-10 bg-gradient-to-br from-white via-slate-50 to-slate-100 dark:from-[#0d0d1f] dark:via-[#0a0f1e] dark:to-[#0f0a1a]">
              ...
              <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div className="max-w-2xl">
                  <div className="gsap-hero-badge mb-4">
                    <LiveClock />
                  </div>
        ...
        <div className="mb-8 grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
          <div className="gsap-kpi"><AuroraStatCard icon={Package}      label="Total Productos"    value={totalProductos} sub="en catálogo activo"    glow="cyan"    delay={0} /></div>
          <div className="gsap-kpi"><AuroraStatCard icon={Archive}      label="Stock Total"        value={totalStock}     sub="unidades en inventario" glow="emerald" delay={0} /></div>
          <div className="gsap-kpi"><AuroraStatCard icon={DollarSign}   label="Valor Inventario"   value={totalValue}     prefix="$" decimals={2}       sub="precio × stock"     glow="violet" delay={0} /></div>
            ...
            <div className="p-6">
              ...
              {topStocked.length === 0 ? (
                ...
                <div className="flex flex-col gap-0.5">
                  {topStocked.map((p, i) => (
                    <StockBar key={p.id} name={p.nombre} current={p.stock || 0} max={topStocked[0]?.stock || 100} index={i} />
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="p-6">
              ...
              {movsRecientes.length === 0 ? (
                ...
                <div className="flex flex-col">
                  {movsRecientes.map((m, i) => <MovRow key={m.id} mov={m} index={i} />)}
                </div>
              )}
            </div>
          </AuroraCard>
