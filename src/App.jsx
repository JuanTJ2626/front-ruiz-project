  ...
  return (
    <div className="dashboard-aurora-bg relative min-h-screen p-6 lg:p-10">
      <div className="admin-header relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <Button onClick={onAdd} className="rounded-xl h-11 px-6 gap-2 shadow-md font-semibold text-[15px]">
          ...
          <div className="p-6 bg-background">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                {!searchQuery && <Button onClick={onAdd} className="mt-6 rounded-xl gap-2"><Plus size={16} /> Agregar Producto</Button>}
              </div>
            ) : viewMode === 'grid' ? (
                      ...
                      <CardContent className="p-5 flex flex-col h-full">
                        <div className="flex justify-between items-start gap-4 mb-3">
                          ...
                          <div className="flex flex-col gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button variant="secondary" size="icon" className="h-8 w-8 rounded-lg" onClick={() => onEdit(prod)}><Pencil size={14} /></Button>
                            <Button variant="destructive" size="icon" className="h-8 w-8 rounded-lg bg-red-100 text-red-600 hover:bg-red-200" onClick={() => onDelete(prod)}><Trash2 size={14} /></Button>
                          </div>
                        </div>
                    ...
                    {filteredProducts.map((prod) => (
                      <TableRow key={prod.id} className="hover:bg-muted/10 transition-colors">
                        ...
                        <TableCell className="text-center">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold ${prod.stock <= 5 ? 'bg-red-100 text-red-700' : 'bg-muted text-muted-foreground'}`}>{prod.stock}</span>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg" onClick={() => onEdit(prod)}><Pencil size={14} className="text-muted-foreground" /></Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg hover:bg-red-100 hover:text-red-600" onClick={() => onDelete(prod)}><Trash2 size={14} /></Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
...
function App() {
  ...
  useEffect(() => {
    if (isAuthenticated && mainRef.current && location.pathname === '/productos') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setEditingId(null); setShowForm(false); cargarProductos();
  ...
  const handleDeleteConfirm = async () => {
    ...
    try { await eliminarProducto(deleteTarget.id); toast.success('Producto eliminado'); cargarProductos(); }
  ...
  if (!isAuthenticated) return (
    <><Toaster position="top-right" richColors /><Login onLogin={() => setIsAuthenticated(true)} /></>
  );
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <Sheet open={showForm} onOpenChange={(open) => !open && cancelForm()}>
        <SheetContent side="right" className="w-[480px] max-w-full flex flex-col p-0 gap-0 border-l">
          <SheetHeader className="px-8 py-6 border-b shrink-0 bg-muted/20">
            ...
            <SheetDescription className="text-[15px]">{editingId ? 'Actualiza la información del producto.' : 'Agrega un nuevo producto al catálogo.'}</SheetDescription>
          </SheetHeader>
      ...
      <main className="flex-1 ml-0 md:ml-40 transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]" ref={mainRef}>
        <Routes>
          ...
          <Route path="/dashboard" element={<Dashboard productos={productos} />} />
          <Route path="/productos" element={
            <ProductosPage
              productos={productos} loading={loading} stats={stats}
              ...
              onAdd={() => { cancelForm(); setShowForm(true); }}
              onEdit={handleEdit} onDelete={setDeleteTarget}
            />
          } />
          <Route path="/stock" element={<StockDashboard productos={productos} />} />
          <Route path="/proveedores" element={<ProveedoresDashboard />} />
          <Route path="/clientes" element={<ClientesDashboard />} />
          <Route path="/reportes" element={<ReportesDashboard productos={productos} />} />
